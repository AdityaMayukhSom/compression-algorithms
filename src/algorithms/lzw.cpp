   ...
   public:
    void compress(const std::string input_file_path, const std::string output_file_path) {
        ...
        if (!input_file.is_open()) {
            std::cerr << "LZW error: unable to open compression input file " << input_file_path << "\n";
            return;
        }

        std::ofstream output_file(output_file_path, std::ios::binary);
        if (!output_file.is_open()) {
            std::cerr << "LZW error: unable to open compression output file " << output_file_path << "\n";
            return;
        }
    ...
    void decompress(const std::string input_file_path, const std::string output_file_path) {
        std::ifstream input_file(input_file_path, std::ios::binary);
        if (!input_file.is_open()) {
            std::cerr << "LZW error: unable to open decompression input file " << input_file_path << "\n";
            return;
        }

        std::ofstream output_file(output_file_path, std::ios::binary);
        if (!output_file.is_open()) {
            std::cerr << "LZW error: unable to open decompression output file " << output_file_path << "\n";
            return;
        }
