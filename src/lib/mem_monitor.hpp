...
class mem_monitor {
   ...
   private:
    ...
    uint64_t extract_number(std::string& line, size_t start_pos, bool extension = false) {
    ...
    void flush() {
        ...
        for (const auto& s : m_stats) {
            m_os << duration_cast<milliseconds>(s.timestamp - m_start_timestamp).count() + 1 << ";"  // round up!
                 << s.pid << ";" << s.VmPeak << ";" << s.VmSize << ";" << s.VmHWM << ";" << s.VmRSS << ";" << s.VmData
                 << ";" << s.VmPTE << ";" << '"' << m_event_names[s.event_id] << '"' << "\n";
        }
        m_stats.clear();
    }
   ...
   public:
    ...
    mem_monitor(const std::string& file_name, milliseconds granularity = milliseconds(50))
